...
# ====================================================================
# generation_id  : 325
# filename        : gen_0325_opus46_10.py
# timestamp       : 2026-05-07T17:34:19Z
...
def check_horizontal(board, row, col):
    return check_direction(board, row, col, 0, 1)


def check_vertical(board, row, col):
    return check_direction(board, row, col, 1, 0)


def check_diagonal_down_right(board, row, col):
    return check_direction(board, row, col, 1, 1)


def check_diagonal_down_left(board, row, col):
    return check_direction(board, row, col, 1, -1)
...
def check_win(board):
    for r in range(NUM_ROWS):
        for c in range(NUM_COLS):
            if board[r][c] == ' ':
                continue
            if check_horizontal(board, r, c):
                return board[r][c]
            if check_vertical(board, r, c):
                return board[r][c]
            if check_diagonal_down_right(board, r, c):
                return board[r][c]
            if check_diagonal_down_left(board, r, c):
                return board[r][c]
    return None
