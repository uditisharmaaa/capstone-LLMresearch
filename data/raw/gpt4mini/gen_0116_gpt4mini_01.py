...
# ====================================================================
# generation_id  : 116
# filename        : gen_0116_gpt4mini_01.py
...
def is_valid_input(column_input):
    if len(column_input) != 1 or column_input not in 'ABCDEFG':
        return False
    return True
...
def main():
    ...
    while True:
        print_board(board)
        column_input = input(f"Player {PLAYER_SYMBOLS[current_player]}, choose a column (A-G): ").upper()
        
        while not is_valid_input(column_input):
            column_input = input("Invalid input. Choose a column (A-G): ").upper()
        
        column = ord(column_input) - ord('A')
        
        if not drop_checker(board, column, PLAYER_SYMBOLS[current_player]):
