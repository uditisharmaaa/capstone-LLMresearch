...
# ====================================================================
# generation_id  : 255
# filename        : gen_0255_gemini_05.py
# timestamp       : 2026-04-06T10:26:15Z
...
def get_player_move(board, player):
    while True:
        col_str = input(f"Player {PLAYER_MARKS[player]}, enter column (A-G): ").upper()
...
def drop_checker(board, col, player):
    for row in range(ROWS - 1, -1, -1):
        if board[row][col] == ' ':
            board[row][col] = PLAYER_MARKS[player]
            return row
...
def check_win(board, row, col, player):
    ...
    for i in range(-3, 4):
        r, c = row + i, col + i
        if 0 <= r < ROWS and 0 <= c < COLS and board[r][c] == mark:
            count += 1
            if count == 4:
                return True
        else:
