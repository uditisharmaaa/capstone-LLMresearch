...
# ====================================================================
# generation_id  : 103
# filename        : gen_0103_gemini_03.py
# timestamp       : 2026-04-06T09:53:04Z
...
def get_player_move(board, player):
    """Gets the player's move and validates input."""
    while True:
        ...
        col = ord(col_str) - ord('A')  # Convert letter to column index

        if not (0 <= col < COLS):
            print("Invalid input. Please enter a letter A-G.")
            continue

        if board[0][col] != ' ':  # Check if column is full
...
def check_draw(board):
    ...
    for row in range(ROWS):
        for col in range(COLS):
            if board[row][col] == ' ':
                return False  # If there is any empty cell, it's not a draw
