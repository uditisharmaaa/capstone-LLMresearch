...
# ====================================================================
# generation_id  : 351
# filename        : gen_0351_o4mini_06.py
# timestamp       : 2026-05-07T17:46:16Z
...
def is_valid_column(col):
    return col in range(COLS) and board[0][col] == EMPTY
...
def get_player_input():
    while True:
        col = input(f"Player {current_player + 1} ({players[current_player]}), choose a column (A-G): ").strip()
        if len(col) == 1 and col in COLUMN_LABELS and is_valid_column(COLUMN_LABELS.index(col)):
...
while True:
    ...
    row = drop_checker(col, players[current_player])
    print_board()
    if check_win(row, col, players[current_player]):
        print(f"Player {current_player + 1} ({players[current_player]}) won!")
        break
